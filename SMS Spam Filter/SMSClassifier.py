...
def kNNModel(k, x_train, x_test, y_train, y_test):
    ...
    for kn, descriptor in model.items():
        # Fit data, get prediction, and calculate accuracy
        descriptor.fit(x_train, y_train)
        prediction = descriptor.predict(x_test)
        print("Predictions: ") 
        print(prediction)
        print("")
...
# Use kNN classifier
kNNModel(7, sms_x_train, sms_x_test, sms_y_train, sms_y_test)